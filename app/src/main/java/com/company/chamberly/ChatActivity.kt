    ...
    class ChatActivity : ComponentActivity(){
        ...
        override fun onCreate(savedInstanceState: Bundle?) {
            ...
            sendButton.setOnClickListener {
                ...
                val message = Message(uid!!, messageText, "text", senderName!!)
                ...
                database.getReference(groupChatId).child("messages").push().setValue(message)
                    .addOnSuccessListener {
                        ...
                        createNotificationDocument(uid, messageText) // Correct place to call this function
                    }
        ...
        private fun fetchRecipientId(completion: (String) -> Unit) {
            ...
            database.reference.child(groupChatId).child("Users").child("members")
                .addListenerForSingleValueEvent(object : ValueEventListener {
                    ...
                    override fun onCancelled(error: DatabaseError) {
                        Log.e("ChatActivity", "Error fetching chat members: ${error.message}")
                    }
                })
        }

        private fun createNotificationDocument(senderId: String, message: String) {
            fetchRecipientId { recipientId ->
                Log.d(TAG, "Recipient ID: $recipientId")
                firestore.collection("Accounts").document(recipientId).get()
                    .addOnSuccessListener { document ->
                        val recipientToken = document.getString("fcmToken") // Make sure this field name matches your Firestore document
                        if (recipientToken != null) {
                            ...
                            firestore.collection("notifications").add(notificationData)
                                .addOnSuccessListener {
                                    Log.d(TAG, "Notification document added successfully")
                                }
                                .addOnFailureListener { e ->
                                    Log.w(TAG, "Error adding notification document", e)
                                }
                        } else {
                            Log.w(TAG, "Recipient FCM token is null")
                        }
                    }
                    .addOnFailureListener { e ->
                        Log.w(TAG, "Error fetching recipient's FCM token", e)
                    }
            }
        }
        ...
        private fun sendRegistrationToServer(token: String) {
            // Implement network call to send token to your server
            ...
            val request = Request.Builder()
                .url("https://your-server.com/api/register_token") // Replace with your server URL
                .post(requestBody)
                .build()

            client.newCall(request).enqueue(object : Callback {
                override fun onFailure(call: Call, e: IOException) {
                    Log.e(MyFirebaseMessagingService.TAG, "Failed to send token to server: $e")
                }
